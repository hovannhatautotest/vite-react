...
DN-09 Verify that Đăng nhập successfully accout Admin with valid Email and Mật khẩu
    [Tags]    @smoketest    @regression
    When Login to admin
    Then User look message "Đăng nhập thành công" popup
    And User look title "Tra cứu dữ liệu"
    And User look menu "Tra cứu dữ liệu"
...
DN-13 Verify that validation text of "Email" field display when "Email" field empty
    ...
    Then Required message "routes.auth.reset-password.Recovery Email" displayed under "Đây là trường bắt buộc!" field
...
DN-15 Verify that can back to login page with "Quay trở lại Đăng nhập" link
    [Tags]    @smoketest    @regression
    When Click "Quên mật khẩu?" link
    When Click "Quay trở lại Đăng nhập" link 
